package com.example.webanttes.data
...
data class Resource<T>(
    ...
    /**
     * Метод для выполнения части [block] в скоупе если текущее состояние равно [ERROR].
     *
     * @param block - блок для выполнения.
     * @return текущий ресурс.
     */
    suspend fun ifErrorAsync(block: suspend (error: Throwable) -> Unit): Resource<T> {
        if (status.isError()) {
            block(error!!)
        }
        return this
    ...
    /**
     * Метод для выполнения части [block] в скоупе если текущее состояние равно [SUCCESS].
     *
     * @param block - блок для выполнения.
     * @return текущий ресурс.
     */
